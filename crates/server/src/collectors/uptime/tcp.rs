...
impl Options {
    fn from_target(target: &Target) -> Result<Self, ProbeError> {
        ...
        Ok(Self { host, port, timeout: tags::parse_timeout(target, "timeout_seconds")? })
    }
}
...
async fn connect(report: &mut Report, options: &Options) {
    ...
    let address = match resolved {
        Err(_) => return report.fail(Failure::Timeout, "name resolution too slow"),
        Ok(Err(error)) => {
            return report.fail(Failure::Dns, format!("{}: {error}", options.host));
        }
        Ok(Ok(mut addresses)) => match addresses.next() {
            Some(address) => address,
            None => {
                return report
                    .fail(Failure::Dns, format!("{} resolves to no address", options.host));
            }
        },
    };
...
mod tests {
    ...
    async fn un_port_ferme_produit_un_echantillon_a_zero_et_non_une_erreur() {
        ...
        let cible = cible("tcp", &format!("127.0.0.1:{port}"), &[("timeout_seconds", "2")]);
        let samples = TcpCollector::new().probe(&cible).await.expect("une mesure, pas une erreur");
