...
/// Longueur au-delà de laquelle une valeur trouvée est tronquée dans le message.
///
/// Le message d'échec part dans les journaux et dans l'interface : y recopier la
/// valeur entière ferait de la sonde un moyen de lire n'importe quel document JSON
/// joignable par le serveur — le chemin suffit à viser un secret. Trente-deux
/// caractères disent assez pour corriger l'attente, pas assez pour l'exfiltrer.
pub const VALUE_LABEL_MAX_CHARS: usize = 32;

/// Forme lisible d'une valeur JSON, pour le message d'échec, jamais plus longue
/// que [`VALUE_LABEL_MAX_CHARS`].
pub fn value_label(value: &Value) -> String {
    let full = match value {
        Value::String(text) => text.clone(),
        other => other.to_string(),
    };
    match full.char_indices().nth(VALUE_LABEL_MAX_CHARS) {
...
mod tests {
    ...
    #[test]
    fn la_valeur_trouvee_est_tronquee_dans_le_message() {
        let secret = "A".repeat(40);
        let label = value_label(&Value::String(secret.clone()));
        assert_eq!(label, format!("{}…", "A".repeat(32)));
        assert!(!label.contains(&secret));

        let long_object = json(&format!(r#"{{"cle":"{}"}}"#, "B".repeat(60)));
        assert_eq!(value_label(&long_object).chars().count(), 33);
        // Une coupe au milieu d'un caractère multi-octets ne doit pas paniquer.
        let accents = Value::String("é".repeat(50));
        assert_eq!(value_label(&accents).chars().count(), 33);
    }
}
