...
| Module | Rôle |
|---|---|
| `metrics.rs` | Cluster, nœuds, invités (listes), stockages, certificats. |
| `guest.rs` | Détail d'une VM (`status/current` : ballon, agent) et systèmes de fichiers via l'agent QEMU. |
| `disks.rs` | Disques physiques (santé SMART, usure, température) et pools ZFS. |
| `apt.rs` | Mises à jour (dont sécurité), changements de versions entre deux interrogations, abonnement, dépôts. |
| `backup.rs`, `snapshots.rs`, `replication.rs`, `ha.rs`, `ceph.rs` | Ce que leur nom dit. |

Le collecteur garde un état entre deux interrogations : les versions de
paquets par cible et par nœud (`ProxmoxCollector::packages`), pour publier
`node_packages_changed` avec le résumé du changement pendant une heure.
...
d'extraits de réponses en constantes, copiés tels quels de réponses
de Proxmox VE 8 ; `client.rs` teste la traduction des statuts HTTP en
