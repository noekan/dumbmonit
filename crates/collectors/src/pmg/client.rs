//! Accès HTTP à l'API Proxmox Mail Gateway.
//!
...
//! sans serveur en face. L'API de PMG reprend les conventions de celle de PVE
//! (enveloppe `{"data": …}`, ticket dans un cookie, jeton dans `Authorization`),
//! seuls les noms changent : `PMGAuthCookie`, `PMGAPIToken`. Le client `reqwest`
//! lui-même vient de `crate::http`, partagé avec les autres collecteurs REST.
...
impl PmgClient {
    ...
    /// Comme [`Self::get`], mais un 403 — et un 404, pour une version de PMG qui
    /// ne connaît pas encore le chemin — donne `Ok(None)` au lieu d'une erreur.
    ///
    /// Pour les appels qui demandent un privilège que l'on ne veut pas exiger, ou
    /// un paquet qui peut ne pas être installé : ClamAV désactivé, pas d'agrégat,
    /// pas d'abonnement. Cela ne doit compter ni comme erreur de collecte ni comme
    ...
    async fn send(
        ...
        request = match &self.auth {
            ...
            AuthMode::Ticket { .. } => {
                ...
                request.header(reqwest::header::COOKIE, format!("PMGAuthCookie={ticket}"))
            }
        };
    ...
    async fn request_ticket(
        ...
        // PMG accepte aussi bien un formulaire qu'un corps JSON : le JSON évite
        ...
        if !status.is_success() {
            ...
            return Err(match status {
                StatusCode::UNAUTHORIZED | StatusCode::FORBIDDEN => ProbeError::Auth(
                    "Proxmox Mail Gateway rejected the credentials (username, password or realm)"
                        .to_string(),
                ),
...
/// ligne ». Un certificat refusé est classé en configuration, sans quoi toute
/// passerelle en certificat auto-signé s'afficherait comme éteinte.
fn map_transport(error: &reqwest::Error, path: &str, timeout: Duration) -> ProbeError {
    ...
    if is_certificate_error(error) {
        return ProbeError::Config(
            "TLS certificate rejected: a Proxmox Mail Gateway installation uses a \
...
fn status_error(status: StatusCode, body: &str, path: &str) -> ProbeError {
    match status {
        StatusCode::UNAUTHORIZED => ProbeError::Auth(
            "Proxmox Mail Gateway refused authentication: invalid or revoked token, or wrong \
             credentials"
                .to_string(),
        ),
        StatusCode::FORBIDDEN => ProbeError::Auth(format!(
            "Insufficient permissions on {path}: give the monitoring user the \"Audit\" role \
             in Configuration → User Management"
        )),
        // Une 5xx est bien une indisponibilité du service : PMG répond mais son
...
mod tests {
    ...
    fn un_401_est_une_erreur_dauthentification_et_pas_une_panne() {
        ...
        assert!(!error.means_down(), "un identifiant invalide ne doit réveiller personne");
    }

    #[test]
    fn un_403_oriente_vers_le_role_manquant() {
        let error = status_error(StatusCode::FORBIDDEN, "", "/statistics/mail");
        assert!(matches!(error, ProbeError::Auth(_)));
    ...
    fn une_4xx_ordinaire_est_une_erreur_de_protocole() {
        let error = status_error(StatusCode::NOT_FOUND, "no such node", "/nodes/x/status");
