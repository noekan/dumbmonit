...
pub struct GroupSummary {
    ...
    /// Nom de l'invité, d'après les notes du dernier instantané (PVE y écrit le
    /// nom de la machine) ; `None` si les notes sont vides ou répètent le groupe.
    pub name: Option<String>,
    /// Les instantanés les plus récents, du plus récent au plus ancien, pour le
    /// calendrier. Plafonné à [`MAX_SNAPSHOTS_PER_GROUP`].
...
pub fn summarize_groups(
    ...
    // Date des notes retenues par groupe, pour ne garder que les plus récentes.
    let mut named: BTreeMap<(&str, &str), i64> = BTreeMap::new();
    ...
    for snapshot in snapshots {
        ...
        summary.count += 1;
        summary.snapshots.push(SnapshotView {
            time,
            size: snapshot.size.map(|s| s.0),
            verified,
            protected: snapshot.protected.is_some_and(|p| p.0 != 0.0),
        });
        ...
        if summary.count == 1 || time > summary.last_time {
            ...
            summary.last_verified = verified;
        }
        // Le nom vient des notes les plus récentes qui en portent un : un
        // instantané sans notes ne fait pas oublier le nom de la machine.
        if let Some(name) = guest_name(snapshot.comment.as_deref(), backup_type, backup_id)
            && named.get(&(backup_type.as_str(), backup_id.as_str())).is_none_or(|&t| time > t)
        {
            named.insert((backup_type.as_str(), backup_id.as_str()), time);
            summary.name = Some(name);
        }
    }

    for summary in groups.values_mut() {
        summary.snapshots.sort_by_key(|s| std::cmp::Reverse(s.time));
        summary.snapshots.truncate(MAX_SNAPSHOTS_PER_GROUP);
    }
...
///
/// PVE écrit par défaut le nom de la machine ; un utilisateur peut y mettre
/// un paragraphe. On ne garde que la première ligne, et seulement si elle
/// apporte quelque chose que `type/id` ne dit pas déjà.
fn guest_name(comment: Option<&str>, backup_type: &str, backup_id: &str) -> Option<String> {
    let line = comment?.lines().next()?.trim();
    if line.is_empty() || line == backup_id || line == format!("{backup_type}/{backup_id}") {
        return None;
    }
    Some(line.chars().take(80).collect())
}

/// Les groupes tels que la vue les livre, dans l'ordre des étiquettes.
pub fn group_views(groups: &BTreeMap<GroupKey, GroupSummary>) -> Vec<GroupView> {
    groups
        .iter()
        .map(|(key, summary)| GroupView {
            datastore: key.datastore.clone(),
            namespace: key.namespace.clone(),
            backup_type: key.backup_type.clone(),
            backup_id: key.backup_id.clone(),
            name: summary.name.clone(),
            count: summary.count,
            last_time: summary.last_time,
            last_size: summary.last_size,
...
/// Les tâches telles que la vue les livre. Une tâche sans `upid` ne peut ni
/// être suivie d'une interrogation à l'autre ni voir son journal : elle est
/// ignorée, comme une tâche sans type.
pub fn task_views(tasks: &[TaskEntry]) -> Vec<TaskView> {
    tasks
        .iter()
        .filter_map(|task| {
            let upid = task.upid.as_deref().filter(|u| !u.is_empty())?;
            let worker_type = task.worker_type.as_deref().filter(|t| !t.is_empty())?;
            Some(TaskView {
                upid: upid.to_string(),
                worker_type: worker_type.to_string(),
                worker_id: task.worker_id.clone().unwrap_or_default(),
                user: task.user.clone(),
                start: task.starttime.map_or(0, |s| s.0 as i64),
                end: task.endtime.map(|e| e.0 as i64),
...
pub struct TaskDigest {
    ...
    /// Issue de la GC la plus récente terminée dans la fenêtre, par datastore :
    /// `(date de démarrage, succès)`. Sert de repli à `gc_last_run_ok` pour un
    /// PBS antérieur à 3.3, dont `/gc` ne dit pas comment s'est fini le dernier
    /// passage.
    pub last_gc_outcome: BTreeMap<String, (i64, bool)>,
}
...
pub fn digest_tasks(tasks: &[TaskEntry], now_s: i64, lookback_s: i64) -> TaskDigest {
    ...
    for task in tasks {
        ...
        if family(worker_type) == Family::GarbageCollection
            && let Some(store) = task.datastore()
        {
            let outcome = (start, task.succeeded());
            digest
                .last_gc_outcome
...
/// Repli de `gc_last_run_ok` d'après les tâches, pour les datastores dont `/gc`
/// ne dit pas l'issue du dernier passage (`known` énumère ceux qui l'ont déjà
/// dite, pour ne pas publier deux fois la même série).
pub fn gc_outcome_samples(
    digest: &TaskDigest,
    ...
    digest
        .last_gc_outcome
        .iter()
        .filter(|(store, _)| !known.contains(*store))
        .map(|(store, (_, ok))| {
            gauge("gc_last_run_ok", if *ok { 1.0 } else { 0.0 }, ts_ms)
                .with_label("datastore", store.clone())
        })
        .collect()
}
...
mod tests {
    ...
    #[test]
    fn la_vue_dun_groupe_garde_ses_instantanes_et_le_nom_de_linvite() {
        let snapshots: Vec<SnapshotEntry> = extraire(SNAPSHOTS);
        let groups = summarize_groups("main", "", &snapshots);
        let views = group_views(&groups);
        assert_eq!(views.len(), 3);

        let vm100 = views.iter().find(|g| g.backup_id == "100").unwrap();
        assert_eq!(
            vm100.name.as_deref(),
            Some("nextcloud"),
            "les notes les plus récentes qui en portent"
        );
        assert_eq!(vm100.count, 3);
        let times: Vec<i64> = vm100.snapshots.iter().map(|s| s.time).collect();
        assert_eq!(
            times,
            vec![1700604800, 1700302400, 1700000000],
            "du plus récent au plus ancien"
        );
        assert!(vm100.snapshots[1].protected);
        assert_eq!(vm100.snapshots[0].verified, Some(true));

        let vm101 = views.iter().find(|g| g.backup_id == "101").unwrap();
        assert_eq!(vm101.snapshots[0].verified, Some(false));
        assert!(vm101.name.is_none(), "sans notes, pas de nom");
        assert_eq!(
            guest_name(Some("vm/100"), "vm", "100"),
            None,
            "les notes qui répètent le groupe n'apportent rien"
        );
        assert_eq!(
    ...
    #[test]
    fn les_taches_de_la_vue_ont_toutes_un_upid() {
        let tasks: Vec<TaskEntry> = extraire(TASKS);
        let views = task_views(&tasks);
        assert_eq!(views.len(), tasks.len());
        let running = views.iter().find(|t| t.worker_id == "main:vm/102").unwrap();
        assert!(running.end.is_none() && running.status.is_none());
        assert_eq!(views[0].user.as_deref(), Some("pve@pbs!token"));
        assert!(
            task_views(&[TaskEntry { worker_type: Some("backup".into()), ..Default::default() }])
                .is_empty()
        );
    }

    #[test]
    fn lissue_de_la_derniere_gc_sert_de_repli_avant_pbs_3_3() {
        let tasks: Vec<TaskEntry> = extraire(TASKS);
        let digest = digest_tasks(&tasks, 1700606000, 24 * 3600);
        assert_eq!(digest.last_gc_outcome["main"], (1700590000, true));
        assert_eq!(digest.last_gc_outcome["archive"], (1700591000, false));

        let known = std::collections::BTreeSet::from(["main".to_string()]);
        let samples = gc_outcome_samples(&digest, &known, 1);
        assert!(
            valeur(&samples, r#"pbs_gc_last_run_ok{datastore="main"}"#).is_none(),
            "déjà dit par /gc"
        );
        assert_eq!(valeur(&samples, r#"pbs_gc_last_run_ok{datastore="archive"}"#), Some(0.0));
    }
