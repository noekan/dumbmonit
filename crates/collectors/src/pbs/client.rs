...
//! lui-même vient de `crate::http`, partagé avec les autres
//! collecteurs REST.
...
impl PbsClient {
    ...
    pub async fn get<T: DeserializeOwned>(
        ...
        self.get_envelope(path, query).await.map(|envelope| envelope.data)
    }

    /// Comme [`Self::get`], en gardant l'enveloppe : les listes paginées — le
    /// journal d'une tâche — y annoncent leur longueur totale.
    pub async fn get_envelope<T: DeserializeOwned>(
        &self,
        path: &str,
        query: &[(&str, String)],
    ) -> Result<Envelope<T>, ProbeError> {
