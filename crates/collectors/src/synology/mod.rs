...
pub use devices::{AbbHistory, DeviceReport, DeviceRun, MemoryHistory};
use options::Options;
...
pub struct SynologyCollector {
    ...
    /// Où les exécutions Active Backup par appareil sont gardées entre deux
    /// interrogations (voir [`devices`]). En mémoire par défaut ; le serveur y
    /// branche SQLite pour que l'historique survive à un redémarrage.
    abb_history: Arc<dyn AbbHistory>,
}

impl Default for SynologyCollector {
    fn default() -> Self {
        Self {
            http_verified: OnceLock::new(),
            http_unverified: OnceLock::new(),
            sessions: Mutex::new(HashMap::new()),
            abb_history: Arc::new(MemoryHistory::default()),
        }
    }
}
...
impl SynologyCollector {
    ...
    /// Remplace le magasin d'exécutions Active Backup par appareil.
    pub fn with_abb_history(mut self, history: Arc<dyn AbbHistory>) -> Self {
        self.abb_history = history;
        self
    }
...
impl Collector for SynologyCollector {
    ...
    async fn probe(&self, target: &Target) -> Result<Vec<Sample>, ProbeError> {
        ...
        if !options.abb {
            ...
            let index = tasks.as_ref().map(|tasks| devices::task_index(tasks)).unwrap_or_default();
            outcome.absorb(
                target.id,
                "tâches Active Backup for Business",
                tasks.map(|tasks| abb::task_samples(&tasks, now_s, ts_ms)),
            );
            // Le rythme par appareil demande l'aperçu des transferts, qu'un ABB
            // ancien n'annonce pas : les tâches restent surveillées sans lui.
            if dsm.supports(devices::API_OVERVIEW) {
                // Les jours actifs et l'heure habituelle s'apprécient à l'heure
                // locale du serveur : c'est celle de la personne qui lit.
                let offset = i64::from(chrono::Local::now().offset().local_minus_utc());
                let reports = devices::refresh(
                    &dsm,
                    self.abb_history.as_ref(),
                    target,
                    &index,
                    now_s,
                    offset,
                ...
                outcome.absorb(
                    target.id,
                    "appareils Active Backup for Business",
                    reports.map(|reports| devices::device_samples(&reports, now_s, ts_ms)),
                );
            } else {
