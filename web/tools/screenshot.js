...
const pages = [
  ['overview', '/'], ['devices', '/targets'], ['device-4', '/targets/4'], ['device-1', '/targets/1'],
  ['new', '/targets/new'], ['new-snmp', '/targets/new?kind=snmp'], ['edit-4', '/targets/4/edit'],
  ['alerts', '/alerts'], ['settings', '/settings'], ['docs', '/docs/notifications'], ['login', '/login'], ['setup', '/setup']
];
(async () => {
  const browser = await puppeteer.launch({ executablePath: '/usr/bin/chromium-browser', args: ['--no-sandbox', '--disable-gpu'] });
  const page = await browser.newPage();
  if (COOKIE) await page.setCookie({ name: 'ezymonit_session', value: COOKIE, domain: 'localhost', path: '/', httpOnly: true });
  for (const theme of ['light', 'dark']) {
    for (const [w, h, tag] of [[1440, 900, 'desktop'], [390, 844, 'mobile']]) {
      ...
      for (const [name, path] of pages.filter(([n]) => (ONLY.length ? ONLY.includes(n) : !(COOKIE && gated.has(n))))) {
        await page.evaluateOnNewDocument((t) => { try { localStorage.setItem('ezymonit-theme', t); } catch {} }, theme);
