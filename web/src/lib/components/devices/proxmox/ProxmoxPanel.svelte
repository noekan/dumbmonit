<script lang="ts">
	 ...
	 * sortable by any column, filtered by node and status. A row unfolds into
	 * its CPU and memory over the page's time range.
	 *
	...
	function fillTone(percent: number | null): 'signal' | 'advisory' | 'warning' {
		if (percent === null) return 'signal';
		if (percent >= 90) return 'warning';
		if (percent >= 75) return 'advisory';
		return 'signal';
	}
	const FILL: Record<'signal' | 'advisory' | 'warning', string> = {
		signal: 'bg-signal',
								...
								<tr class={`rise-in border-t border-line/60 ${open ? 'bg-surface-2/40' : 'hover:bg-surface-2/40'}`} style="--rise-delay: {Math.min(i, 8) * 30}ms">
									...
									<td class="px-3 py-2 align-middle">
										<button type="button" class="flex min-w-0 flex-col text-left" onclick={() => toggleRow(g.vmid)}>
											...
											<span class="tnum text-[0.75rem] text-ink-2">{g.kind === 'lxc' ? 'CT' : 'VM'} {g.vmid}</span>
										</button>
									</td>
									<td class="px-3 py-2 align-middle">
